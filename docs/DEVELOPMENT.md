...
How to run, build and test the project locally. Phase 1 covers only the skeleton; no auth, WebSocket or business features.
...
- **backend/** – Spring Boot app; packages: `common`, `health`, `auth`, `room`, `chat`, `todo`, `event`, `realtime`, `persistence` (modular monolith skeleton).
- **frontend/** – React + TypeScript + Vite; single page calling `/api/health`.
- **docs/** – Development guide, architecture, ADRs.

## Overall Plan

- **Phase 1:** Skeleton, Docker, health endpoint, minimal frontend, docs (current).
- **Phase 2:** Auth (e.g. JWT), WebSocket/realtime, rooms and chat.
